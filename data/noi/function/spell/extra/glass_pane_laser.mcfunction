...
execute positioned ^ ^ ^.5 positioned ~-.25 ~-.25 ~-.25 as @e[tag=!noi.laser_ignore,dx=0,dy=0,dz=0,tag=noi.target,sort=random] run tag @s add current

execute positioned ^ ^ ^.5 run function noi:spell/extra/glass_pane_laser